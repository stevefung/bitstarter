...
var buildResponseHandler = function(checksfile) {
    var handleResponse = function(result, response) {
        if (result instanceof Error) {
            console.error('Error: ' + util.format(response.message));
        } else {
            //handle result
            outputResult(checkHtml(cheerio.load(result), checksfile));
        }
    };
...
if (require.main == module) {
    ...
    if (program.url) {
        // pull from URL
        restler.get(program.url).on('complete', buildResponseHandler(program.checks));
    } else {
        // use local file
